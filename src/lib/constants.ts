...
export const FIRM = {
  ...
  foundedYear: Number(process.env.NEXT_PUBLIC_FIRM_FOUNDED_YEAR ?? "2024"),

  // ── ICAI registration details (placeholders — replace with the firm's real values) ──
  frn: process.env.NEXT_PUBLIC_FIRM_FRN ?? "161031W",            // ICAI Firm Registration No.
  ...
  address: {
    line1: process.env.NEXT_PUBLIC_ADDRESS_LINE1 ?? "Shreeji Skyrise, A/203, Swami Vivekanand Rd",
    line2: process.env.NEXT_PUBLIC_ADDRESS_LINE2 ?? "Opp. Fire Brigade, Parekh Nagar, Kandivali West",
    city: process.env.NEXT_PUBLIC_ADDRESS_CITY ?? "Mumbai",
    state: process.env.NEXT_PUBLIC_ADDRESS_STATE ?? "Maharashtra",
    pin: process.env.NEXT_PUBLIC_ADDRESS_PIN ?? "400 067",
    country: process.env.NEXT_PUBLIC_ADDRESS_COUNTRY ?? "India",
  },

  phone: process.env.NEXT_PUBLIC_PHONE ?? "+91 82372 92639",
  email: process.env.NEXT_PUBLIC_EMAIL ?? "caankitshah21@gmail.com",
  whatsapp: process.env.NEXT_PUBLIC_WHATSAPP ?? "918237292639",
  officeHours: process.env.NEXT_PUBLIC_OFFICE_HOURS ?? "Mon – Sat: 10:00 AM – 7:00 PM",
  calcom: process.env.NEXT_PUBLIC_CALCOM_URL ?? "https://cal.com/ca-ankit-shah",
  googleMapsEmbed: process.env.NEXT_PUBLIC_GOOGLE_MAPS_EMBED ?? "",
};

// ── Social Media ──────────────────────────────────────────────────────────────
export const SOCIAL = [
  { label: "LinkedIn", icon: "linkedin", href: process.env.NEXT_PUBLIC_SOCIAL_LINKEDIN ?? "https://in.linkedin.com/in/ca-ankit-shah-90646720a" },
  { label: "Instagram", icon: "instagram", href: process.env.NEXT_PUBLIC_SOCIAL_INSTAGRAM ?? "https://instagram.com/ca_ankit_shah_21" },
  { label: "WhatsApp", icon: "whatsapp", href: process.env.NEXT_PUBLIC_SOCIAL_WHATSAPP ?? `https://wa.me/${FIRM.whatsapp}` },
] as const;
...
export const SERVICES = [
  ...
  },
  {
    ...
    shortDesc: "Business registrations, legal documentation, and allied advisory across your operations.",
    desc: "Beyond core accountancy, we support a range of allied and legal-support services — business and startup registrations, drafting and vetting of agreements, RERA, trademark/IPR and other statutory registrations, labour-law compliance, and liaison with regulatory authorities — provided in coordination with legal professionals where required.",
    tags: ["Registrations", "Legal Documentation", "RERA / IPR", "Labour Law", "Regulatory Liaison"],
  },
] as const;
